...
class Version:
    """Simple semantic version implementation without external dependencies"""
    ...
    def __init__(self, version_string: str):
        ...
        # Parse semantic version: major.minor.patch[-pre_release][+build]
        match = re.match(
        ...
        if match:
            ...
            # Fallback: try to parse just major.minor.patch
            ...
            if len(parts) >= 2:
                try:
                    ...
                    pass
            if len(parts) >= 3:
                # Handle patch version that might have pre-release suffix
    ...
    def __lt__(self, other) -> bool:
        ...
        # If versions are equal, pre-release versions are lower
    ...
    def get_releases(self, repo_id: str) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                logger.warning(
                    f"Failed to fetch releases for {repo_id}: {response.status_code}"
                )
                return []
    ...
    def get_latest_release(self, repo_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                logger.warning(
                    f"Failed to fetch latest release for {repo_id}: {response.status_code}"
                )
                return None
...
class RegistryManager:
    ...
    def get_packages_by_filter(
        self, external_only: bool = False, nodetool_ai_only: bool = False
    ) -> List[Dict]:
        ...
        if external_only:
            return [
                pkg
                for pkg in packages
                if not pkg.get("repo_id", "").startswith("nodetool-ai/")
            ]
        elif nodetool_ai_only:
            return [
                pkg
                for pkg in packages
                if pkg.get("repo_id", "").startswith("nodetool-ai/")
            ]
        else:
            return packages
...
def parse_version(tag_name: str) -> Optional[Version]:
