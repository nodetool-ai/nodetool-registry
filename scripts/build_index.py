...
from registry_utils import GitHubAPIClient, RegistryManager, has_wheel_assets, parse_version, setup_logging
...
class NodeToolRegistryBuilder:
    ...
    def get_wheel_metadata(self, asset_url: str, asset_name: str) -> Dict:
        ...
        try:
            ...
            # Generate hash for integrity (in production, you'd download and hash)
            # For now, using URL as seed for reproducible hash
            url_hash = hashlib.sha256(asset_url.encode()).hexdigest()[:16]
            
            return {
                'url': asset_url,
                'filename': asset_name,
                'size': size,
                'hash': f'sha256={url_hash}',  # Placeholder - should be real hash
    ...
    def generate_package_page(self, package_name: str, repo: str, output_dir: Path):
        ...
        for wheel in wheels:
            ...
            if wheel.get('hash'):
                attrs.append(f'data-dist-info-metadata="sha256={wheel["hash"]}"')
            ...
            # Add core metadata
            attrs.append(f'data-core-metadata="sha256={wheel.get("hash", "unknown")}"')
    ...
    def generate_root_index(self, output_dir: Path, package_counts: Dict[str, int]):
        ...
        for package_name in sorted(self.packages.keys()):
            ...
            total_wheels += count
            html_lines.append(f'  <a href="{package_name}/">{package_name}</a> ({count} wheels)<br>')
        ...
        print(f"✅ Generated root index ({len(self.packages)} packages, {total_wheels} wheels)")
    
    def build_index(self, output_dir: str, force_rebuild: bool = False, package_filter: str = None):
        ...
        if package_filter:
            packages_to_build = {k: v for k, v in self.packages.items() 
                               if k == package_filter}
        ...
        print(f"🌐 Usage: pip install --index-url file://{output_path.absolute()}/ nodetool-base")

def main():
    parser = argparse.ArgumentParser(description='Build NodeTool package index')
    parser.add_argument('--output-dir', default='dist', help='Output directory for index')
    parser.add_argument('--github-token', help='GitHub token for API access')
    parser.add_argument('--force-rebuild', action='store_true', help='Force rebuild entire index')
    parser.add_argument('--package-filter', help='Only build index for specific package')
